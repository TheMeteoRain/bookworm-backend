...
CREATE TABLE book (`book_id` INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,`publisher_id` INT UNSIGNED,`title` VARCHAR(255) NOT NULL,`description` TEXT,`genre` VARCHAR(150) NOT NULL,`format` VARCHAR(150)  NOT NULL,`price` DECIMAL(5,2) NOT NULL,`stock` SMALLINT NOT NULL,`pages` SMALLINT UNSIGNED NOT NULL,`isbn` VARCHAR(15) NOT NULL,CONSTRAINT `fk_book_publisher` FOREIGN KEY (`publisher_id`) REFERENCES `publisher`(`id`) ON UPDATE CASCADE ON DELETE SET NULL) ENGINE=InnoDB  DEFAULT CHARSET=utf8;
